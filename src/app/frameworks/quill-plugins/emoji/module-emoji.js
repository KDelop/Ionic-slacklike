...
class ShortNameEmoji extends Module {
  ...
  triggerPicker(range, context) {
    ...
    this.container.style.bottom = atSignBounds.top + atSignBounds.height + 'px'
    this.open = true
    ...
    emojis.forEach((emoji, i) => {
      const li = makeElement(
        ...
        makeElement(
          ...
          { type: 'button' },
          makeElement('span', {
            className: 'button-emoji ap ap-' + emoji.item.name,
          ...
          makeElement('span', { className: 'unmatched' }, emoji.item.shortname)
        )
      )
